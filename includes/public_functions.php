...
// Return username on receiving user id
function getUsernameById($id){
	global $conn;
	$result = mysqli_query($conn, "SELECT username FROM users WHERE id=" . $id . "limit 1");

	// Return username
	return mysqli_fect_assoc($result)['username'];
}

// Return username on receiving comment id
function getRepliesByCommentId($id){
	global $conn;
	$result = mysqli_query($conn, "SELECT * FROM replies WHERE comment_id=$id");
	$replies = mysqli_fecth_all($result, MYSQLI_ASSOC);
	return $replies;
}

// Return total number of comment on a post on receiving the post's id
